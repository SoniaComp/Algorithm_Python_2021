...
MAX_LOOP = 10 ** 8
BINARY_DIR_BASE = os.path.join(os.path.dirname(
    os.path.abspath(__file__)), '../ext_code')
...
def timeit_lp(func, func_args, funcs=None, num_iter=100, time_limit=0.1, log=False,
    ...
    """
    timeit_msg = timeit(func, func_args, num_iter=num_iter,
                        time_limit=time_limit, return_msg=True, silence=silence)
    lp_msg = lprun(func, func_args, funcs=funcs,
                   return_msg=True, silence=silence)
    if log:
...
def timeit(func, func_args, num_iter=100, time_limit=0.1, silence=False, return_msg=False):
    elapse_times = _calc_elapse_times(
        func, func_args, num_iter=num_iter, time_limit=time_limit)
...
def timeits(func, func_args_list, num_iter=100, time_limit_per_args=0.1, silence=False, return_msg=False):
    ...
    for func_args in func_args_list:
        e_times = _calc_elapse_times(
            func, func_args, num_iter=num_iter, time_limit=time_limit_per_args)
...
def time_complexity(func: Callable, args_func: Callable, scales: Iterable, num_iter=10, time_limit=1):
    ...
    for scale in scales:
        ...
        cur_elapse_times = _calc_elapse_times(
            func, args, num_iter=num_iter, time_limit=time_limit)
...
def make_elapse_time_msg(elapse_times):
    ...
    msg = '==> avg_time: {} in {} iterations'.format(
        _to_time_str(avg_time), len(elapse_times))
    return msg
